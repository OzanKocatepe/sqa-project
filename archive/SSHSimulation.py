import numpy as np
from .SSH import SSH
from .data.SSHParameters import EnsembleParameters, ModelParameters
from .data import *
...
class SSHSimulation:
    ...
    def Run(self, tauAxisDim: np.ndarray[float], initialConditions: np.ndarray[complex], numT: int=10, steadyStateCutoff: float=25, debug: bool=False):
        ...
        # Defines the start and end of a period in seconds.
        ...
        tAxisSec = np.linspace(startPoint, endPoint, numT)
        ...
        for kIndex, tup in enumerate(self.__models.items()):
            k, model = tup
            args.append((k, kIndex, model, len(list(self.__models.items())), tauAxisDim, initialConditions, numT, steadyStateCutoff, debug))
    ...
    def _RunWrapper(self, argTuple):
        k, kIndex, model, modelsSize, tauAxisDim, initialConditions, numT, steadyStateCutoff, debug = argTuple
        print(f"Solving for momentum {k / np.pi:.2f}pi ({kIndex + 1}/{modelsSize})...")
        model.Solve(tauAxisDim, initialConditions, numT, debug=False)
