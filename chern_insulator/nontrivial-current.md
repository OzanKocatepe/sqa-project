...
Numerical solution for sigma matches the Fourier series, and then obviously multiplying by current coefficients results in the same steady state solution for the current as well, so the Fourier series seems to be working fine.
